...
This document contains scripts for automating the Azure cross-tenant log collection setup process. Scripts are provided in both **PowerShell** (recommended for Azure Cloud Shell) and **Python** formats. These scripts complement the main guide ([azure-cross-tenant-log-collection-guide.md](azure-cross-tenant-log-collection-guide.md)).
...
2. [Choose Your Script Version](#choose-your-script-version)
3. [Step 0: Register Resource Providers (PowerShell)](#step-0-register-resource-providers-powershell)
4. [Step 0: Register Resource Providers (Python)](#step-0-register-resource-providers-python)
...
| **Authentication** | Azure PowerShell (Connect-AzAccount), Azure CLI, or Service Principal |

### Step-by-Step Execution

**For PowerShell (Recommended for Azure Cloud Shell):**
...
1. **Authenticate to the SOURCE tenant**:
   ```bash
...
## Choose Your Script Version

| Environment | Recommended Script | Why |
|-------------|-------------------|-----|
| **Azure Cloud Shell (PowerShell)** | ✅ PowerShell | Az module pre-installed, no setup needed |
| **Azure Cloud Shell (Bash)** | ⚠️ PowerShell (switch to PS) | Python SDK not pre-installed |
| **Local Windows** | PowerShell or Python | Both work with proper setup |
| **Local Mac/Linux** | Python | Native Python support |

### Quick Decision Guide

- **Using Azure Cloud Shell?** → Use the **PowerShell script** (Section 3)
- **Have Azure CLI working locally?** → Use the **Python script** (Section 4)
- **Only Connect-AzAccount works?** → Use the **PowerShell script** (Section 3)

---

## Step 0: Register Resource Providers (PowerShell)
...
The Az PowerShell module is **pre-installed** in Azure Cloud Shell. For local use:
...
## Step 0: Register Resource Providers (Python)

> ⚠️ **Note**: This version requires Python and Azure SDK packages. For Azure Cloud Shell, use the PowerShell version above.

This Python script checks and registers the `Microsoft.ManagedServices` resource provider across all subscriptions in a tenant. This is required before deploying Azure Lighthouse.
